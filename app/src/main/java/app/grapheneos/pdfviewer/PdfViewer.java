...
public class PdfViewer extends AppCompatActivity {
    ...
    private final AtomicBoolean documentPropertiesLoaded = new AtomicBoolean(false);
    ...
    private final ActivityResultLauncher<Intent> openDocumentLauncher = registerForActivityResult(
            new ActivityResultContracts.StartActivityForResult(), result -> {
                ...
                if (resultData != null) {
                    viewModel.setUri(resultData.getData());
                    resetDocumentState();
                    loadPdf();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState == null && Intent.ACTION_VIEW.equals(intent.getAction())) {
            ...
            viewModel.setUri(intent.getData());
            resetDocumentState();
        }
    ...
    public boolean onPrepareOptionsMenu(@NonNull Menu menu) {
        ...
        final boolean hasPages = viewModel.getNumPages() > 0;
        setMenuItemState(menu.findItem(R.id.action_jump_to_page), loaded && hasPages,
                enabled && hasPages);
        setMenuItemState(menu.findItem(R.id.action_next), loaded && hasPages,
                enabled && hasPages && viewModel.getPage() < viewModel.getNumPages());
        setMenuItemState(menu.findItem(R.id.action_previous), loaded && hasPages,
                enabled && hasPages && viewModel.getPage() > 1);
        setMenuItemState(menu.findItem(R.id.action_first), loaded && hasPages, enabled && hasPages);
        setMenuItemState(menu.findItem(R.id.action_last), loaded && hasPages, enabled && hasPages);
